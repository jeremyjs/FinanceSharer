package edu.purdue.cs.cs252.channel;
...
/**
 * A TCP implementation of the channel. It depends on having a single server and
 * multiple clients. The Server is always listening for messages (connections).
 * Clients, once initialized, register at the server. A client can send a
 * message to the server immediately. while servers have to wait until the
 * client checks the server (mailbox) for new messages. Clients cannot
 * communicate with each other, instead they have to do it through the server.
 * 
 * @author fmeawad
 * 
 */
public class TCPChannel extends Channel implements Runnable {
    ...
    public TCPChannel(String server, int port) throws ChannelException {
        ...
        while (this.channelID == -1)
            try {
                ...
                throw new ChannelException(
                        "Error while waiting for the new channelID", e);
            }
        ...
        clientTimer.scheduleAtFixedRate(new TimerTask() {
            ...
            public void run() {
                try {
                    sendMessage(new Message(Message.Type.CheckMessage, "",
                            channelID));
    ...
    protected void sendMessage(Message message) throws ChannelException {
        ...
        if (this.server != null) {
            ...
            pendingMessages.get(message.getChannelID()).add(message);
    }
    ...
    public void run() {
        ...
        while (true) {
            ...
            try {
                ...
                System.err.println("Unable to accept connection 1: "
                        + e.getMessage());
                break;
            } catch (IOException e) {
                System.err.println("Unable to accept connection 2: "
                        + e.getMessage());
                break;
            } catch (ChannelException e) {
                e.printStackTrace();
